...
const BannerList = () => {
  ...
  const { bannerList, bannerPagination, isLoading, isDeleting } = useSelector(
  ...
  const debouncedDispatch = useCallback(
    debounce(() => {
      dispatch(getBannerListAction(currentPage, dataLimit, searchText));
    }, 500),
  ...
  const getActionButtons = (newsmedia: any) => {
    ...
    if (hasPermission("department.edit")) {
      actions.push({
        element: "Edit",
        onClick: () => router.push(`/banner/edit?id=${newsmedia.id}`),
        iconClass: "pencil",
      });
    }
    ...
    <div>
      ...
      <PageContentList>
        {isLoading ? (
          ...
          <Table
            ...
            dataLimit={dataLimit}
            totalData={bannerPagination.total}
          >
            {bannerList && bannerList.length > 0 ? (
              bannerList.map((newmedia: any, index: number) => (
                <tr
      ...
      <PermissionModal
        ...
        handleAction={() => {
          if (messageID) {
            dispatch(
              deleteNewsMedia(messageID, () => setShowDeleteModal(false))
            );
          }
        }}
      />
